...
void igvMallaTriangulos::calcularNormalesVertices() {
    ...
    // 1 y 2: Calcular normal de cada triángulo y acumular en vértices
    for (long int t = 0; t < num_triangulos; t++) {
        ...
        // Producto vectorial (normal del triángulo)
        float nx = v1y * v2z - v1z * v2y;
        float ny = v1z * v2x - v1x * v2z;
        float nz = v1x * v2y - v1y * v2x;

        // Acumular en los tres vértices del triángulo
        normales[i0 * 3 + 0] += nx;
        normales[i0 * 3 + 1] += ny;
        normales[i0 * 3 + 2] += nz;
    ...
    // 3 y 4: Normalizar cada vector normal
